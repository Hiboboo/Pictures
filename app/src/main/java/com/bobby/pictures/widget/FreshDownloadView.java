...
public class FreshDownloadView extends View
{
    ...
    public FreshDownloadView(Context context, AttributeSet attrs, int defStyleAttr)
    {
        ...
        circular_edge = getResources().getDimension(R.dimen.dp_8);
    ...
    private void parseAttrs(TypedArray array)
    {
        if (array != null)
        {
            try
            {
                setRadius(array.getDimension(R.styleable.FreshDownloadView_circular_radius, getResources().getDimension(R.dimen.dp_8)));
...
//                    status = STATUS.DOWNLOADING;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh)
    {
        ...
        initArrowPath(top, left, getRadius());
    }
    ...
    private void initArrowPath(int top, int left, float radius)
    {
    ...
    private void drawDownLoading(Canvas canvas, RectF arcBounds)
    {
        ...
        if (progress_degree >= 1.0f)
            status = STATUS.DOWNLOADED;
        Log.i(TAG, "绘制下载中的进度=" + progress_degree);
    }
    ...
    private void drawText(Canvas canvas, float progress_degree)
    {
        ...
        canvas.drawText("%", rect.centerX() + textBounds.width() / 2 + .1f * radius, baseline, publicPaint);
        Log.i(TAG, "绘制文字=" + progress_degree);
    }
    ...
    synchronized void setProgressInternal(float progressInternal)
    {
        ...
        Log.i(TAG, "更新进度 =" + mProgress + "/" + status);
        if (status == STATUS.PREPARE)
            startDownload();
    ...
    private AnimatorSet getDownLoadErrorAnimator()
    {
        ...
        animator1.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
        {
            ...
            public void onAnimationUpdate(ValueAnimator animation)
            {
                mErrorLeftDegree = (float) animation.getAnimatedValue();
                invalidate();
            }
        });
    ...
    static class FreshDownloadStatus extends AbsSavedState
    {
        ...
        public static final Parcelable.Creator<FreshDownloadStatus> CREATOR = new Parcelable.Creator<FreshDownloadStatus>()
        {
            @Override
